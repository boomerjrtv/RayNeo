...
object RayNeoProtocol {
    ...
    fun classifyPacket(raw: ByteArray): RayNeoPacket {
        if (raw.size < 4) return RayNeoPacket.Unknown
        return when (raw[1].u()) {
            PKT_SENSOR -> parseSensorSample(raw)?.let { RayNeoPacket.Sensor(it) } ?: RayNeoPacket.Unknown
            PKT_RESPONSE -> if (raw.size <= RESPONSE_CMD_OFFSET) RayNeoPacket.Unknown
                            else RayNeoPacket.Response(raw[RESPONSE_CMD_OFFSET].u(), raw)
            else -> RayNeoPacket.Unknown
        }
    }

    fun parseDeviceInfo(raw: ByteArray): RayNeoDeviceInfo? {
        if (raw.size <= DEVINFO_SIDE_BY_SIDE_OFFSET) return null
    ...
    fun selectEndpoints(device: UsbDevice): RayNeoEndpointSelection? {
        ...
        for (i in 0 until device.interfaceCount) {
            ...
            for (j in 0 until usbInterface.endpointCount) {
                ...
                if (ep.type == UsbConstants.USB_ENDPOINT_XFER_CONTROL) continue
                if (ep.direction == UsbConstants.USB_DIR_IN && inEndpoint == null) inEndpoint = ep
                else if (ep.direction == UsbConstants.USB_DIR_OUT && outEndpoint == null) outEndpoint = ep
            }
            if (inEndpoint != null && outEndpoint != null) {
                val sel = RayNeoEndpointSelection(usbInterface, inEndpoint, outEndpoint)
                if (inEndpoint.type == UsbConstants.USB_ENDPOINT_XFER_INT &&
                    outEndpoint.type == UsbConstants.USB_ENDPOINT_XFER_INT) return sel
                if (fallback == null) fallback = sel
            }
        }
        return fallback
    }
    ...
    private fun parseSensorSample(raw: ByteArray): RayNeoSensorSample? {
        if (raw.size < MAG_Z_OFFSET + 4) return null
        return RayNeoSensorSample(
            accelMps2 = floatArrayOf(readF32LE(raw, ACCEL_OFFSET), readF32LE(raw, ACCEL_OFFSET + 4), readF32LE(raw, ACCEL_OFFSET + 8)),
            gyroDps   = floatArrayOf(readF32LE(raw, GYRO_OFFSET),  readF32LE(raw, GYRO_OFFSET + 4),  readF32LE(raw, GYRO_OFFSET + 8)),
            temperatureC = readF32LE(raw, TEMP_OFFSET),
            magnet    = floatArrayOf(readF32LE(raw, MAG_X_OFFSET), readF32LE(raw, MAG_X_OFFSET + 4), readF32LE(raw, MAG_Z_OFFSET)),
    ...
    private fun readU32LE(raw: ByteArray, offset: Int): Int {
        return raw[offset].u() or (raw[offset+1].u() shl 8) or (raw[offset+2].u() shl 16) or (raw[offset+3].u() shl 24)
    }

    private fun readF32LE(raw: ByteArray, offset: Int): Float = Float.fromBits(readU32LE(raw, offset))
}
...
class RayNeoPacketAssembler {
    ...
    fun append(data: ByteArray, count: Int) {
        if (count <= 0) return
        if (count >= stash.size || len + count > stash.size) { len = 0 }
    ...
    fun nextPacket(): ByteArray? {
        var offset = 0
        while (offset + 4 <= len) {
            if (stash[offset].toInt() and 0xFF != RayNeoProtocol.PKT_MAGIC) { offset++; continue }
            val packetLen = stash[offset + 2].toInt() and 0xFF
            if (packetLen < 4) { offset++; continue }
            if (offset + packetLen > len) break
            val packet = Arrays.copyOfRange(stash, offset, offset + packetLen)
            compact(offset + packetLen)
            return packet
        }
        if (offset > 0) compact(offset)
        return null
    }

    private fun compact(consumed: Int) {
        if (consumed <= 0) return
        if (consumed >= len) { len = 0; return }
